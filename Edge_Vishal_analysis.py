...
try:
    ap = wks.worksheet("Vishal Analysis")
    wks.del_worksheet(ap)
except:
    print('none')

try:
    worksheet = wks.add_worksheet(title="Vishal Analysis", rows="1", cols="2")
except:
    worksheet = gc.open("Daily_Task_Inhouse").worksheet("Vishal Analysis")
